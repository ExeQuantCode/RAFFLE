...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  !! 0 b 0
  !! 0 0 c
  double precision, allocatable, dimension(:,:) :: peakseparation, temppeaks
  double precision, allocatable, dimension(:) :: peaksindividual

  double precision, dimension(3,3) :: box
  double precision, dimension(3) :: angle, spacelist, tmpvector, bestlocation, backuplocation
  double precision, dimension(:,:,:,:), allocatable :: bondlist
  double precision ::  bondcutoff,connectivity,tmpangle, volmin, volmax, bondpro1, bondpro2, distribution, tmpvalue
  double precision :: anglecutoffupper, anglecutofflower, sigma2, bestlocationindex, agausssamp, peak1, tmpdistribution
  double precision :: angle_distribution, bond_distribution, normalisation_a, prob_void, prob_scan, prob_pseudo
     ...
     do i=1, eltot 
        do j=1, eltot
           if(i.eq.j) then 
              ...
              !print*, dble(stochio(i)/leng)**2, elnames(i), elnames(j)

           else 
              if(j.lt.i) cycle
              normalisation_a=normalisation_a+dble(dble(stochio(i)+stochio(j))/leng)**2
              !print*, dble(dble(stochio(i)+stochio(j))/leng)**2, elnames(i), elnames(j)
     ...
     do i=1, eltot
        ...
        do j=1, eltot
           ...
           if(i.eq.j) then 
              ...
              !     &sphereoverlap(elrad(2,i,i),elrad(2,j,j),elrad(1,i,j),pi))
              !print*, leng**2-bonding_number_correction, stochio(i)*stochio(j)
...
!!! CHECKS IF THE UNIT CELL WILL FORCE ATOMS TO BE TOO CLOSE TOGETHER
     do i=1, eltot 
        do k=1, eltot
           do j=1, 3
              !print*, (formula(structures)%cell(j,1)**2+formula(structures)%cell(j,2)**2+formula(structures)%cell(j,3)**2)**0.5, &
              ...
              if((formula(structures)%cell(j,1)**2+formula(structures)%cell(j,2)**2+formula(structures)%cell(j,3)**2)**0.5&
                 ...
                 print*, "This unit cell would definitely cause recursive atoms to be closer together than 0.9 Elrad"
...
!!! Assigns all the atoms to the correct species label. 
     ...
     do i=1+z, leng
        !print*, i, z, "£"
        if(i.eq.1+z) then
           !print*, "This is the first new atom, thus elnames", addedelements+1, "should be ", atomlist(structures,i)%name
           elnames(addedelements+1)=atomlist(structures,i)%name 
           !print*, "As this is the first atom, stochio (", addedelements +1, ") = 1"
           ...
           if(atomlist(structures,i)%name.eq.atomlist(structures,i-1)%name) then 
              ...
              !print*, atomlist(structures,i)%name, "is the name of atom", i, " which is equal to", &
              !&atomlist(structures,i-1)%name, "which is name of atom", i-1
              !print*, "Thus, the count of", size(stochio), " is incremented by", 1
           else 
              !print*, "!"
        ...
        !print*, atomlist(structures,i)%name
     end do
     ...
     !do i=1, leng 
     !   print*,  atomlist(structures,i)%element_index,  atomlist(structures,i)%name
     !end do
     
     !do i=1, leng*27
     !   print*, alistrep(structures,i)%element_index
     !end do
     ...
     if(modeselect.eq.2) then;
        ...
        !print*, L
        do i=1,L 
           !print*, atomlist(structures,i)%position(:) 
        end do
        ...
        do i=1, L*27
           !print*, alistrep(structures,i)%position, L*27 
        end do
...
!!!!!!!!!!!!! This section places the atoms via distribution !!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     aloop: do while (i.le.leng-1)
        ...
        prob_void=dble(tmpdig(1)/dble(tmpdig(1)+tmpdig(2)+tmpdig(3)))
        prob_pseudo=prob_void+dble(tmpdig(2)/dble(tmpdig(1)+tmpdig(2)+tmpdig(3)))
        prob_scan=prob_pseudo+dble(tmpdig(3)/dble(tmpdig(1)+tmpdig(2)+tmpdig(3)))
...
!         end if
        ...
        print*, prob_void, prob_pseudo, prob_scan 
...
!Generate the atomic bonding information files used in upcoming learning algorithm
        ...
        !end do
        !do i=1, leng
           !print*, atomlist(structures,i)%position, i
        !end do
...
!  end do
  ...
  do j=1, 3
      write(structures+10000,*) formula(structures)%cell(j,1), &
           formula(structures)%cell(j,2), &
           formula(structures)%cell(j,3)
      !print*, formula(structures)%cell(j,1), &
      !     formula(structures)%cell(j,2), &
      !     formula(structures)%cell(j,3)
...
!call poswrite(formula(structures)%cell,alistrep,leng, structures, structno, prev_structures)



write(tmp,'(A11,I0.3)')"pos/POSCAR_",prevpos+structures
call Incarwrite(adjustl(tmp),500, 20*leng)
!print*, structures, prevpos, "!!!!!!!!!!!!!!"
...
close(77)
  ...
  subroutine addhost(eltot,structures,formula,location,atomlist,stochio,elnames,&
    ...
    double precision :: cellmultiplier,meanvol,tmpdble
    ...
    !print*, formula(1)%cell
   !! Probably incorrect to do this step here
    !meanvol=cellvol(formula(structures)%cell)
    !!!!!!!!!!!!!
    ecount=0
...
!!!!WARNING ONLY FOR ONE SYSTEM USE WITH CAUTION
    ...
    do i=1, eltot
       ...
       read(50,*)
       do j=1, tmp
          !print*, trim(adjustl(elnames(i))), trim(adjustl(elnames_list(j)))
