...
TYPE reason_matrix
   character(1024) :: reason
end type reason_matrix
...
subroutine buildmap_POINT(tmpvector,formula,atomlist,alistrep&
     ...
     &,uptol,lowtol,calculated_value,override,current_bests)
  ...
  double precision, dimension(3) :: tmpvector, location_vector, current_bests
  double precision, dimension(:,:,:), allocatable :: elrad
  ...
  allocate(predicted_positions(1,27))
  ...
  elloop:do el=1, maxval(atomlist(structures,:)%element_index)
     ...
     call atomprojector(tmpvector,predicted_positions,formula,atom_number_previous+1,structures)
     !do i=1, 27 
     !   print*, predicted_positions(1,i)%position, i
     !end do

     !do i=1, 27 
     !   print*, alistrep(structures,i)%position, i
     !end do
     m_count=0
     n_count=0
     norm=1
     do L=1, atom_number_previous*27+26
        comparison=100.0*tmpvector(3)/formula(structures)%cell(3,3)
        !print*, comparison, c_cut, c_min
        ...
        if(bondlength(tmpvector,&
           ...
           !end if
           !print*, "t-1 bond too short"!, tmpvector, position_storage(1,:) 
           cycle elloop
           !!New trial line of cutoff at static 5A, shoudl correspond to BL cut in dist function
        else if(bondlength(tmpvector,&
             &position_storage(1,:)).gt.&
             &5.0) then
!            &elrad(1,atomlist(structures,atom_number_previous+1)%element_index,index_storage(1,1))*uptol*2) then
...
!!!! This has been left out to ease on computation. Could be reimplemented but increases cost dramatically to consider ALL atoms
           ...
           !end if
            ...
            if(norm.eq.1) then
               t_body_matrix(el)=value_return
            else
               !print*, t_body_matrix(el),value_return
               t_body_matrix(el)=min(t_body_matrix(el),value_return)
            end if
        ...
        if(bondlength(&
           ...
           do n=1, atom_number_previous*27+26
              ...
              if(bondlength(&
                 ...
                 !if((i.eq.2).and.(j.eq.2).and.(k.eq.5)) then
                 !print*, "-------------------------------"
                 !print*, value_return,th_body_matrix(el), "ang1"
                 !print*, tmpvector,position_storage(1,:),position_storage(2,:)

                 !end if

                 
                 if(current_bests(2).gt.value_return) then 
                    calculated_value=0
                    !print*, "ANGLE", value_return, "LESSER THAN BEST", current_bests(2)
                 ...
                 if(n_count.eq.1) then 
                    th_body_matrix(el)=value_return
                 else
                    th_body_matrix(el)=min(th_body_matrix(el),value_return)               
                 end if
                 ...
                 !if((i.eq.2).and.(j.eq.2).and.(k.eq.5)) then
                 !print*, "-------------------------------"
                 !print*, value_return, th_body_matrix(el), "ang2"
                 !print*, tmpvector,position_storage(1,:),position_storage(2,:)
                 !print*, bondangle(&
                 !     &position_storage(1,:),&
                 !     &tmpvector,&
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                 ...
                 do m=1, atom_number_previous*27+26
                    ...
                    if(bondlength(&
                         &position_storage(1,:),&
                         &position_storage(3,:)).lt.&
                         elrad(1,index_storage(1,1),index_storage(3,1))*lowtol) then
                    ...
                    if(bondlength(&
                       ...
                       !if(f_body_matrix(el)*value_return.eq.0) then
                       !   f_body_matrix(el)=&
                       !        &max(f_body_matrix(el), value_return)
                       !else
                       ...
                       if(current_bests(3).gt.value_return) then
                          calculated_value=0
                          !print*, "4b", value_return, "LESSER THAN BEST", current_bests(3)
                       ...
                       if(m_count.eq.1) then
                          f_body_matrix(el)=value_return
                       else
                          f_body_matrix(el)=min(f_body_matrix(el),value_return)
                       end if
                 ...
                 do m=1, atom_number_previous*27+26
                    ...
                    if(bondlength(&
                         &tmpvector,&
                         &position_storage(3,:)).lt.&
                         elrad(1,atomlist(structures,atom_number_previous+1)%element_index,index_storage(3,1))*uptol) then
                       ...
                       call evaluate_4body_contribution(trim(adjustl(&
                            ...
                            &value_return)
                       !if((i.eq.2).and.(j.eq.2).and.(k.eq.5)) then
                       !print*, value_return, f_body_matrix(el),"4b"
                       !end if
                       ...
                       !if(f_body_matrix(el)*value_return.eq.0) then
                       !   !print*, "WORKED", f_body_matrix(el),value_return
                       !   f_body_matrix(el)=&
                       !        &max(f_body_matrix(el), value_return)
                       !
                       !else
     ...
     if(n_count.ge.1) current_bests(2)=th_body_matrix(el)
     if(m_count.ge.1) current_bests(3)=f_body_matrix(el)
     ...
     if(override(2).eqv..TRUE.)  th_body_matrix(el)=1
     if(override(3).eqv..TRUE.)  f_body_matrix(el)=1
     ...
     print*, tmpvector, t_body_matrix(el),th_body_matrix(el),f_body_matrix(el) 
   ...
   !print*,summation,t_body_matrix(el), th_body_matrix(el),f_body_matrix(el)
   !print*, tmpvector, product_matrix(el),f_body_matrix(el), "!!!" 

   open(101,file="POINTFILE",access="APPEND")
   write(101,*) tmpvector, t_body_matrix(el), th_body_matrix(el), f_body_matrix(el), calculated_value
   close(101)
end do elloop
...
type(unitcell), dimension(:), allocatable :: formula
type (atom), dimension(:,:), allocatable :: atomlist,alistrep,predicted_positions
type(reason_matrix), dimension(:,:,:), allocatable :: reason
integer :: el,i,j,k,l,m,n, normalisation,atom_number_previous,structures,&
     &m_count, n_count, atom_total, eltot, cleanup, cleanup2, repeats, num_VOID, c_cut, norm&
     &,c_min, NEWMETHOD_loop
double precision :: value_return, summation, max_1, max_2, max_3, comparison, uptol, lowtol, normaliser&
     &,repeat_power, i_comp, j_comp, k_comp, totbin, calculated_value
integer, dimension(3) :: sum, bin_size, best
double precision, dimension(3) :: tmpvector, location_vector, best_vector, current_bests
double precision, dimension(:,:,:), allocatable :: elrad
double precision, dimension(:,:,:,:), allocatable :: update_region,f_body_matrix, th_body_matrix, t_body_matrix, product_matrix
double precision, dimension(:,:), allocatable :: position_storage
integer, dimension(:,:), allocatable :: index_storage 
...
logical :: file_exists, placed, do_append, NEWMETHOD
logical, dimension(3) :: override
character(3), dimension(:), allocatable :: elnames
integer, dimension(:), allocatable :: tmpdig
character(1024), dimension(:), allocatable :: tmpels
...
allocate(reason((sum(1)+1),(sum(2)+1),(sum(3)+1)))
...
if(file_exists) then 
   ...
   eloop: do m=1,  maxval(atomlist(structures,:)%element_index)
       do cleanup=1, n
         ...
         if(cleanup.ne.n) then 
            print*, "CLEANUP CYCLE"
            !print*, cleanup, n, remaining_elements(1,cleanup)
            cycle eloop
         end if
      end do
         ...
         do j=0, sum(2)
            do k=0, sum(3) 
               ...
               logic_loop2 : do L=(atom_number_previous-num_VOID)*27+1,(atom_number_previous)*27
                  !print*, location_vector,alistrep(structures,L)%position
                  if(bondlength(location_vector,alistrep(structures,L)%position)&
                       &.lt.(elrad(3,m,&
                       &alistrep(structures,L)%element_index)*3.0)) then 
                     !print*, i, j, k , m,1
      ...
      firstloop :do k=0,sum(3)  
         ...
         elloop:do el=1, maxval(atomlist(structures,:)%element_index)
            ...
            if(update_region(i+1,j+1,k+1,el).ne.1) then
               !print*, "CYCLE C",update_region(i+1,j+1,k+1,el),elnames(el), i, j, k
               cycle elloop
            end if
            if(atomlist(structures,atom_number_previous+1)%element_index.ne.el) cycle elloop            
            

            tmpvector(1)=dble(i*dble(formula(structures)%cell(1,1)/(sum(1))))
            tmpvector(2)=dble(i*dble(formula(structures)%cell(1,2)/(sum(1))))
            tmpvector(3)=dble(i*dble(formula(structures)%cell(1,3)/(sum(1))))
            tmpvector(1)=dble(tmpvector(1)+j*dble(formula(structures)%cell(2,1)/(sum(2))))
            tmpvector(2)=dble(tmpvector(2)+j*dble(formula(structures)%cell(2,2)/(sum(2))))
            tmpvector(3)=dble(tmpvector(3)+j*dble(formula(structures)%cell(2,3)/(sum(2))))
            tmpvector(1)=dble(tmpvector(1)+k*dble(formula(structures)%cell(3,1)/(sum(3))))
            tmpvector(2)=dble(tmpvector(2)+k*dble(formula(structures)%cell(3,2)/(sum(3))))
            tmpvector(3)=dble(tmpvector(3)+k*dble(formula(structures)%cell(3,3)/(sum(3))))
...
!This chops. Do not enable chop unless you want to chop
            ...
            NEWLOOP: if(NEWMETHOD.eqv..TRUE.) then 
               override=.FALSE.
               if(NEWMETHOD_LOOP.eq.1) override(3)=.TRUE.  
               if(NEWMETHOD_LOOP.eq.2) override(2)=.TRUE.
            ...
            !print*, tmpvector, calculated_value
            ...
            do L=1, atom_number_previous*27+26
               ...
               if(L.gt.atom_number_previous*27) then 
                  ...
                  !print*, alistrep(structures,L)%position(:)
                  position_storage(1,:)=alistrep(structures,L)%position(:)
                  index_storage(1,1)=alistrep(structures,L)%element_index 
                  name_storage(1,1)=alistrep(structures,L)%name
                  !print*, position_storage(1,:)
...
!!! T-1 Bond filtered if atom is compared to itself. UPDATE: T bonds shouldn't be filtered? 

               !if(bondlength(tmpvector,&
               !     &position_storage(1,:)).lt.0.1) cycle
               ...
               if(bondlength(tmpvector,&
                  ...
                  !print*, "CYCLE 1", i, j, k
                  reason(i+1,j+1,k+1)%reason="Bonglength t-1 too short"
...
!!!! This has been left out to ease on computation. Could be reimplemented but increases cost dramatically to consider ALL atoms
                  ...
                  !   print*, alistrep(structures,L)%position(:)
                  !end if
                  t_body_matrix(i+1,j+1,k+1,el)=((t_body_matrix(i+1,j+1,k+1,el)*dble(norm))+value_return)
                  !ADD +1 to norm here to curtail contributions to bondlength. I do not think you need to, as it can dampen certain resonance points 

                  norm=norm+1
                  t_body_matrix(i+1,j+1,k+1,el)=dble(t_body_matrix(i+1,j+1,k+1,el)/norm)
...
!!!Tested without self removal. Breaks, as expected. 


                     !if(n.eq.L) cycle

                     ! Replacing index filtering with explicit position checks for reliability 
                     !if(bondlength(&
                     !     &tmpvector,&
                     !     &position_storage(1,:)).lt.&
                     !     &0.1) cycle
...
!!! T-2 and 1-2 bonds filtered if atom is compared to itself
                     if(n.gt.atom_number_previous*27) then
                        if(bondlength(&
                             &tmpvector,&
                             &position_storage(2,:)).lt.&
                     ...
                     if(bondlength(&
                        ...
                        f_body_matrix(i+1,j+1,k+1,el)=0
                        th_body_matrix(i+1,j+1,k+1,el)=0
                        reason(i+1,j+1,k+1)%reason="3-body t-2 below bondcutoff"
                        !print*, "CYCLE 2"
                        cycle elloop
                     end if
                     if(bondlength(&
                     ...
                     reason(i+1,j+1,k+1)%reason="Some angle bonds not accounted for"
                     !if(i.eq.6.AND.j.eq.6.AND.k.eq.10) print*, n,th_body_matrix(i+1,j+1,k+1,el)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                        ...
                        do m=1, atom_number_previous*27+26
                           ...
                           if(bondlength(&
                              ...
                              th_body_matrix(i+1,j+1,k+1,el)=0
                              t_body_matrix(i+1,j+1,k+1,el)=0
                              reason(i+1,j+1,k+1)%reason="First component 4-body 1-3 bondlength too short"
                              cycle elloop
                           end if
                           if(bondlength(&
                                &position_storage(1,:),&
                              ...
                              if(value_return.eq.0) then
                                 ...
                                 !print*, "CYCLE 4"
                                 reason(i+1,j+1,k+1)%reason="First component 4-body return 0"
                                 
                                 !print*, fourbody(&
                                 !  &tmpvector,&
                                 !  &position_storage(1,:),&
                                 !  &position_storage(2,:),&
                                 !  &position_storage(3,:)), i,j,k 
                                 !print*, tmpvector
                                 !print*, position_storage(1,:)
                                 !print*, position_storage(2,:)
                                 !print*, position_storage(3,:)
                                 !print*, bondlength(tmpvector, position_storage(1,:))
                                 !print*, bondlength(tmpvector, position_storage(2,:))
                                 !print*, bondlength(tmpvector, position_storage(3,:))


                                 !print*, bondlength(position_storage(1,:),(position_storage(2,:)))
                                 !print*, bondlength(position_storage(1,:),(position_storage(3,:)))
                              ...
                              if(f_body_matrix(i+1,j+1,k+1,el)*value_return.eq.0) then
                                 f_body_matrix(i+1,j+1,k+1,el)=&
                                   &max(f_body_matrix(i+1,j+1,k+1,el), value_return)
                              else
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                        ...
                        do m=1, atom_number_previous*27+26 
                           !print*, L, m, n
                           !Replacing index checking with explicit bondlength checks
                           !if(m.eq.L) cycle 
                           !if(m.eq.n) cycle
                           ...
                           !!Added .AND. to avoid erroneous comparison of atom to itself.
                           ...
                           if(bondlength(&
                              ...
                              !        &position_storage(3,:))
                              !end if
                              reason(i+1,j+1,k+1)%reason="Second component 4-body t-3 bondlength too short"
                              !print*, tmpvector,&
                              !  &position_storage(3,:), L, m, n
                              cycle elloop
                           end if
                           if(bondlength(&
                                &tmpvector,&
                              ...
                              if(value_return.eq.0) then
                                 ...
                                 !print*, "CYCLE 4"
                                 reason(i+1,j+1,k+1)%reason="Second component 4-body is 0"
                                 cycle elloop
                                 !else
                                 !print*,f_body_matrix(i+1,j+1,k+1,el), value_return, i,j,k
                              end if
                              ...
                              reason(i+1,j+1,k+1)%reason="Second componenet 4-body bondlength t-3 too high"
                        ...
                        !  &bondlength(&
                        !  &position_storage(1,:),&
                        !  &position_storage(2,:)), i, j, k

                        reason(i+1,j+1,k+1)%reason="Didn't find any 4-bonds"
                        
                        !cycle
                         
                     end if
                     
                     reason(i+1,j+1,k+1)%reason="Process completed normally"
                     

                     !if(i.eq.1.AND.j.eq.1.AND.k.eq.0) then 
                     !print*, value_return, f_body_matrix(i+1,j+1,k+1,el),"!!!"
                     !end if


                     !print*, value_return, i,j,k, "*(&^*&^^%*&"
                     !Here have taken a large root of value return, to account for sumamtion of atoms in 3D. 
                     !Will need to think further on this.
                     !Lines moved to individual sections, old code for posterity
...
!!!!NOTE: These two functions check if any 3-4 bodies have been evaluated. If they have not, because the region is "empty" then they should be set to something, but a hard cutoff of setting it to 1 can cause weirdness. It should be small, but normalisation isn't rigorous so neither is this. 
         ...
         if(product_matrix(i+1,j+1,k+1,el).ge.summation) then 
...
print*, summation, best

if(summation.gt.0) exit NML 
if(NEWMETHOD.eqv..FALSE.) exit NML
...
allocate(append_matrix(3,3,3,4,maxval(atomlist(structures,:)%element_index)))



!WIGGLE CONTROL BOOLEAN NEA
do_append=.TRUE.
if(do_append.eqv..TRUE.) then
   ...
   do i= -1, 1
      ...
      do j= -1, 1  
         ...
         do k= -1, 1 
            ...
            !write(6,'(A)',ADVANCE='NO') achar(13)
            i_comp=(L-1)*(sum(2)+1.0)*(sum(3)+1.0)
            j_comp=(m-1)*(sum(3)+1)
            k_comp=(n-1)
            totbin=(sum(1)+1)*(sum(2)+1)*(sum(3)+1)
            !write(6,'(I3.0, A)', ADVANCE='NO') NINT(100*(i_comp+j_comp+k_comp)/totbin), "%"
            ...
            tmpvector(1)=best_vector(1)+dble(i)*formula(structures)%cell(1,1)/dble(sum(1)*100.0)
            tmpvector(2)=best_vector(2)+dble(i)*formula(structures)%cell(1,2)/dble(sum(1)*100.0)
            tmpvector(3)=best_vector(3)+dble(i)*formula(structures)%cell(1,3)/dble(sum(1)*100.0)
            tmpvector(1)=tmpvector(1)+dble(j)*formula(structures)%cell(2,1)/dble(sum(2)*100.0)
            tmpvector(2)=tmpvector(2)+dble(j)*formula(structures)%cell(2,2)/dble(sum(2)*100.0)
            tmpvector(3)=tmpvector(3)+dble(j)*formula(structures)%cell(2,3)/dble(sum(2)*100.0)
            tmpvector(1)=tmpvector(1)+dble(k)*formula(structures)%cell(3,1)/dble(sum(3)*100.0)
            tmpvector(2)=tmpvector(2)+dble(k)*formula(structures)%cell(3,2)/dble(sum(3)*100.0)
            tmpvector(3)=tmpvector(3)+dble(k)*formula(structures)%cell(3,3)/dble(sum(3)*100.0)

            !if(calculated_value.ne.0) then
            !   print*, tmpvector, calculated_value, summation
            !end if

            !if(calculated_value.eq.summation) then 
            !print*, tmpvector, best_vector
            !end if
   ...
   append_matrix=0
   do i=1, 3
      append_matrix(1,1,1,i,atomlist(structures,atom_number_previous+1)%element_index)=best_vector(i)
   end do
   append_matrix(1,1,1,4,atomlist(structures,atom_number_previous+1)%element_index)=summation
...
!best_vector(1)=2.63518
!best_vector(2)=2.46286
!best_vector(3)=0.83164
...
!call buildmap_POINT(best_vector,formula,atomlist,alistrep&
!              &,atom_number_previous,structures,elrad,atom_total,eltot, elnames,placed,num_VOID&
!              &,uptol,lowtol,calculated_value,override,current_bests)
...
!best_vector(1)=2.3079 
!best_vector(2)=2.371855   
!best_vector(3)=0.524217
...
!print*,summation , t_body_matrix(best(1)+1,best(2)+1,best(3)+1,1),&
!                 &th_body_matrix(best(1)+1,best(2)+1,best(3)+1,1),f_body_matrix(best(1)+1,best(2)+1,best(3)+1,1)
...
toploop:  do i=1, 3
   do j=1, 3
      do k=1, 3
         if((append_matrix&
              &(i,j,k,4,atomlist(structures,atom_number_previous+1)%element_index)).ge.&
            ...
            calculated_value=append_matrix(i,j,k,4,atomlist(structures,atom_number_previous+1)%element_index)
            summation=calculated_value
            !print*, summation
            exit toploop
         end if
      end do
...
print*, summation, "SUMMATION", best_vector
if(summation.eq.0) then
   !!Similar to earlier comment on recursive buildmaps. Set to 1 to disable
...
do el=1,  maxval(atomlist(structures,:)%element_index)
   write(name,'(A,I0.3,A,I0,A,I0)') "pos/POSCAR_", structurecounter("pos"),"/heatmap_plotter",el,"_",atom_number_previous+1
...
!         do k=1, 11 
!
   !         write(1060+el,*) append_matrix(i,j,k,:,atomlist(structures,atom_number_previous+1)%element_index)
   !      end do
   !   end do
   !end do
   ...
   do i=0, sum(1)
      do j=0, sum(2)
         do k=0,sum(3)
            ...
            tmpvector(2)=dble(i*dble(formula(structures)%cell(1,2)/sum(1)))
            tmpvector(3)=dble(i*dble(formula(structures)%cell(1,3)/sum(1)))
            tmpvector(1)=tmpvector(1)+dble(j*dble(formula(structures)%cell(2,1)/sum(2)))
            tmpvector(2)=tmpvector(2)+dble(j*dble(formula(structures)%cell(2,2)/sum(2)))
            tmpvector(3)=tmpvector(3)+dble(j*dble(formula(structures)%cell(2,3)/sum(2)))
            tmpvector(1)=tmpvector(1)+dble(k*dble(formula(structures)%cell(3,1)/sum(3)))
            tmpvector(2)=tmpvector(2)+dble(k*dble(formula(structures)%cell(3,2)/sum(3)))
            tmpvector(3)=tmpvector(3)+dble(k*dble(formula(structures)%cell(3,3)/sum(3)))
            ...
            !if((tmpvector(1)-best_vector(1)).lt.0.001) then 
            !   if((tmpvector(2)-best_vector(2)).lt.0.001) then
            !      if((tmpvector(3)-best_vector(3)).lt.0.001) then
            !         do L=1, 3
            !            tmpvector(L)=best_vector(L)
            !         end do
            !         product_matrix(i+1,j+1,k+1,el)=calculated_value
            !      end if
            !   end if
    ...
    !        product_matrix(i+1,j+1,k2*(product_matrix(i+1,j+1,k+1,el)**2/summation**2)
            ...
            !print*, tmpvector(1),tmpvector(2),tmpvector(3), i, j, k, el
            ...
            write(1050,*) tmpvector(1),tmpvector(2),tmpvector(3), product_matrix(i+1,j+1,k+1,el)&
                 &,f_body_matrix(i+1,j+1,k+1,el), th_body_matrix(i+1,j+1,k+1,el), &
                 &t_body_matrix(i+1,j+1,k+1,el), trim(adjustl(reason(i+1,j+1,k+1)%reason))
            !UNCOMMENT ALL FOLLOWING FOR CORRECT PRINTOUT FOR PYTHON CODE. ALSO HARDCODED ELEMENT COLOURS
            ...
            !write(1060+el,*) i,j,k,tmpvector(1),", ", tmpvector(2),", ", tmpvector(3),", ",&
            !     &product_matrix(i+1,j+1,k+1,el), t_body_matrix(i+1,j+1,k+1,el), th_body_matrix(i+1,j+1,k+1,el)&
            !     &,f_body_matrix(i+1,j+1,k+1,el)
...
!print*, "Devolved/",trim(adjustl(element_a))&
...
open(1000, file=trim(adjustl(name)))
...
bondlength_read(2)=bondlength_target
      ...
      read(1000,*,IOSTAT=stat) bondlength_read(3), value_read(3) 
...
master :do while(1.eq.1)
   ...
   IF((angles_read(2).le.angles_read(3)).and.&
        &(angles_read(2).ge.angles_read(1))) then
      !!Assuming the line between points can be modelled as linear
      a=angles_read(1)
      b=angles_read(2)
      c=angles_read(3)
   ...
   IF(IS_IOSTAT_END(stat)) then 
      print*, angles_read(1), angles_read(2), angles_read(3)
      if(angles_read(2).ge.angles_read(3)) then 
         print*, "YOU FOUND IT"
      end if
      exit
   end if
...
subroutine add_atom_scan_2 (bin_size,formula,atomlist,alistrep,atom_number_previous,structures,elrad,&
 ...
 !Hard coded atom colours need changing 
  ...
  do i=0, bin_size-1
     do j=0, bin_size-1
        firstloop :do k=0, bin_size-1 
           tmpvector(1)=i*dble(formula(structures)%cell(1,1)/bin_size)
           tmpvector(2)=i*dble(formula(structures)%cell(2,1)/bin_size)
           tmpvector(3)=i*dble(formula(structures)%cell(3,1)/bin_size)
           tmpvector(1)=tmpvector(1)+j*dble(formula(structures)%cell(1,2)/bin_size)
           tmpvector(2)=tmpvector(2)+j*dble(formula(structures)%cell(2,2)/bin_size)
           tmpvector(3)=tmpvector(3)+j*dble(formula(structures)%cell(3,2)/bin_size)
           tmpvector(1)=tmpvector(1)+k*dble(formula(structures)%cell(1,3)/bin_size)
           tmpvector(2)=tmpvector(2)+k*dble(formula(structures)%cell(2,3)/bin_size)
           tmpvector(3)=tmpvector(3)+k*dble(formula(structures)%cell(3,3)/bin_size)
...
subroutine add_atom_pseudo (bin_size,formula,atomlist,alistrep,atom_number_previous,structures,elrad,&
  ...
  character(3), dimension(:), allocatable :: elnames
  type(unitcell), dimension(:), allocatable :: formula
  integer :: el_correct_read,i, j, k,l, num_VOID,atom_number_previous, new_atom_number, structures, leng,eltot
  integer, dimension(3) :: bin_size
  double precision, dimension(3) :: best_location
  double precision :: best_location_value, distribution, sigma1, sigma2, bondcutoff&
       &,agausssamp, calculated_value, calculated_test, uptol, lowtol, r, crude_norm,n&
       &,loop_counter
  double precision, dimension(3) :: tmpvector, testvector,current_bests
  type (atom), dimension(:,:), allocatable :: atomlist, alistrep, predicted_positions
  double precision, dimension(:,:,:), allocatable :: elrad, product_matrix
  logical :: new_position_needed, placed
  logical, dimension(3) :: override
double precision, dimension(:,:,:,:,:), allocatable :: results_matrix, append_matrix
  ...
  infloop : do 
     ...
     testvector(:)=matmul(formula(structures)%cell,tmpvector(:))
     !print*, testvector
     
     current_bests=0
     call buildmap_POINT (testvector,formula,atomlist,alistrep&
     ...
     infloop2 : do 
        ...
        do j=1, 3
           ...
           if(k.gt.10) then 
              testvector(j)=tmpvector(j)+n*0.01*r
           else
              testvector(j)=tmpvector(j)+n*0.1*r
           end if 
        end do

        
        call buildmap_POINT (matmul(formula(structures)%cell,testvector(:)),formula,atomlist,alistrep&
             &,atom_number_previous,structures,elrad,leng,eltot, elnames,placed,num_VOID&
             &,uptol,lowtol,calculated_test,override,current_bests)
        ...
        print*, matmul(formula(structures)%cell,testvector(:)), calculated_test, crude_norm,calculated_test/crude_norm,k
        call random_number(r)



        !if(calculated_test.gt.0) then
        !   k=k+1
        !   crude_norm=((crude_norm*(k-1))+calculated_value)/dble(k)
        !   print*, calculated_value, i,k, r, tmpvector, crude_norm
        !
        !end if
        ...
        if (r.lt.calculated_test) then 
           placed=.TRUE.
           tmpvector=matmul(formula(structures)%cell,testvector(:))
           exit infloop
        end if
...
subroutine regenerate_distribution_files (prev_structures)
  ...
  call invar(18,tmpdig,tmpels)
  structure_begin=tmpdig(1)
  deallocate(tmpdig)


strucloop:do structure_loop=structure_begin, prev_structures
  !DEBUG
  !strucloop:do structure_loop=303, 304 
    ...
    ! do i=1, size(structure_stochiometry)
    !    do j=1, size(structure_stochiometry)
    !       print*,  bondcutoff(i,j), structure_stochiometry(i),  structure_stochiometry(j)
    !       
    !    end do
    ! end do
    ! stop
     ...
     do i=1, sum(structure_stochiometry)
        print*, i, "This is the atom that is being added"
  ...
  strucloop2:do structure_loop=structure_begin, prev_structures
  ...
  strucloop3:do structure_loop=structure_begin, prev_structures
...
subroutine bond_evolution(mode)
  ...
  integer :: prev_structures, mode,i, nbin, stat, exitst, exitst2, exitst3, structure_begin
  character(1024), dimension(:), allocatable :: tmpels
  integer, dimension(:), allocatable :: tmpdig
  logical :: dir_e
  ...
  if(mode.eq.1) then 
     ...
     do i=structure_begin, prev_structures
  ...
  do while(1.eq.1) 
     ...
     do while(1.eq.1)
        ...
        do i=1, nbin
           dist_height=1/((1+gaussian(1,i)))**2
  ...
  do while(1.eq.1)
     ...
     do i=1, nbin 
        !if(gaussian(2,i).lt.0.25*norma_vector(2)) gaussian(2,i)=0
...
15  integer, dimension(:), allocatable :: b
...
function bondangle(A,B,C) result(theta)
  ...
  if(theta.eq.0) theta=3.141592654

end function bondangle

function fourbody(A,B,C,D) result(theta)
  double precision :: theta, x
  double precision, dimension(3) :: A,B,C,D, bond1, bond2, AB, CB, DB
  ...
  CB(:)=C(:)-B(:)
  DB(:)=D(:)-B(:)
  theta=angle_between_vectors(cross(CB,DB),AB)
  !print*, AB
  !print*, CB 
  !print*, DB
  !print*, cross(CB,DB)
  ...
  !OLD FUNCTION CENTERS ON ATOM A. NEW FUNCTION ON B. 
...
subroutine generate4files(stage,structures,prev_structures,array,repeatedarray,&
  ...
  do ip=1, size(structure_elements)
     if(trim(adjustl(structure_elements(ip))).ne.&
          &trim(adjustl(array(structures,atomnumber)%name))) then
        cycle
     else
        bondint_1=ip
     end if
        ...
        do p=1, 27
           ...
           do ip=1, size(structure_elements)
              if(trim(adjustl(structure_elements(ip))).ne.&
                 ...
                 print*, ip, structure_elements, structure_elements(ip)
                 cycle
              else
                 bondint_2=ip
              end if
           ...
           if(bondlength(array(structures,atomnumber)%position,&
                &repeatedarray(structures,x)%position).lt.0.1) then 
              cycle
           end if
           ...
           do k=1, eltot
              do L=1, stochio(k)
                 do n=1, 27
                    ...
                    if(m.eq.x) cycle
                    if(m.eq.atomnumber) cycle
                    if(bondlength(repeatedarray(structures,atomnumber)%position,&
                         &repeatedarray(structures,m)%position).lt.0.01) cycle
                    if(bondlength(repeatedarray(structures,atomnumber)%position,&
                         &repeatedarray(structures,m)%position).gt.bondlengthcutoff(bondint_1,bondint_3)) cycle
                    if((N.eq.14).and.(bondlength(repeatedarray(structures,m)%position,&
                         &repeatedarray(structures,atomnumber)%position)).eq.0) cycle
                    ...
                    do ii=1, eltot
                       do jj=1, stochio(ii)
                          do kk=1, 27
                             ...
                             if(bondlength(repeatedarray(structures,atomnumber)%position,&
                                  &repeatedarray(structures,LL)%position).gt.bondlengthcutoff(bondint_1,bondint_4)) cycle
                             if((kk.eq.14).and.(bondlength(repeatedarray(structures,LL)%position,&
                                  &repeatedarray(structures,atomnumber)%position)).eq.0) cycle
                             print*, x,atomnumber,m,LL,size(array)
                             ab=array(structures,atomnumber)%position-repeatedarray(structures,x)%position
                             ac=repeatedarray(structures,atomnumber)%position-repeatedarray(structures,m)%position
                             ad=repeatedarray(structures,atomnumber)%position-repeatedarray(structures,LL)%position
                             res=fourbody(repeatedarray(structures,x)%position,&
                                  &array(structures,atomnumber)%position,&
                                  &repeatedarray(structures,m)%position,&
                                  &repeatedarray(structures,LL)%position)
                             if(isnan(res)) cycle
...
!         print*, array(structures,m)%position, m
