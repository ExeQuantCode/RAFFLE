...
!!!! This has been left out to ease on computation. Could be reimplemented but increases cost dramatically to consider ALL atoms
            ...
            if(m_count.eq.0) then 
               f_body_matrix(i+1,j+1,k+1,el)=0.25
               !print*, "4_body set to zero on", tmpvector 
            end if
            if(n_count.eq.0) then 
               !print*, "3_body set to one on", tmpvector
               th_body_matrix(i+1,j+1,k+1,el)=0.25
            end if
...
allocate(append_matrix(sum(1)+1,sum(2)+1,sum(3)+1,4,maxval(atomlist(structures,:)%element_index)))

do i= -floor(dble(sum(1)/2)), ceiling(dble(sum(1)/2))
   l=l+1
   m=0
   do j= -floor(dble(sum(2)/2)), ceiling(dble(sum(2)/2))
      m=m+1
      n=0
      do k= -floor(dble(sum(3)/2)), ceiling(dble(sum(3)/2)) 
         ...
         tmpvector(1)=best_vector(1)+dble(i)*formula(structures)%cell(1,1)/(dble(sum(1)**2))
         tmpvector(2)=best_vector(2)+dble(i)*formula(structures)%cell(1,2)/(dble(sum(1)**2))
         tmpvector(3)=best_vector(3)+dble(i)*formula(structures)%cell(1,3)/(dble(sum(1)**2))
         tmpvector(1)=tmpvector(1)+dble(j)*formula(structures)%cell(2,1)/(dble(sum(2)**2))
         tmpvector(2)=tmpvector(2)+dble(j)*formula(structures)%cell(2,2)/(dble(sum(2)**2))
         tmpvector(3)=tmpvector(3)+dble(j)*formula(structures)%cell(2,3)/(dble(sum(2)**2))
         tmpvector(1)=tmpvector(1)+dble(k)*formula(structures)%cell(3,1)/(dble(sum(3)**2))
         tmpvector(2)=tmpvector(2)+dble(k)*formula(structures)%cell(3,2)/(dble(sum(3)**2))
         tmpvector(3)=tmpvector(3)+dble(k)*formula(structures)%cell(3,3)/(dble(sum(3)**2))
...
subroutine add_atom_pseudo (bin_size,formula,atomlist,alistrep,atom_number_previous,structures,elrad,&
  ...
  double precision, dimension(3) :: best_location
  double precision :: best_location_value, distribution, sigma1, sigma2, bondcutoff&
       &,agausssamp, calculated_value, uptol, lowtol, r, crude_norm
  double precision, dimension(3) :: tmpvector
  type (atom), dimension(:,:), allocatable :: atomlist, alistrep, predicted_positions
  double precision, dimension(:,:,:), allocatable :: elrad, product_matrix
  logical :: new_position_needed, placed
