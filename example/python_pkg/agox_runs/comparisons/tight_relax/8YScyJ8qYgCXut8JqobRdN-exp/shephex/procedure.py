...
def raffle_agox(train=True, n_dist=4, index=42):
    ...
    for i in range(len(template))[::-1]:
        if template[i].position[2] > template.cell[2][2] * 0.15:
            del template[i]
            indices += 1
    ...
    generator = RaffleGenerator(**environment.get_confinement(),
        ...
        train=train,
        seed=seed,
        gen_params = {"void": 0.0, "rand": 0.0, "walk": 0.0, "grow": 0.0, "min": 1.0})
    ...
    evaluator = LocalOptimizationEvaluator(
        ...
        optimizer_run_kwargs={"fmax": 0.025, "steps": 100},
        order=3,
...
if __name__ == "__main__":
    ...
    experiments = (raffle_agox(directory='tight_relax/').
                   zip(train=[True]).
