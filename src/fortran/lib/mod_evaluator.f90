...
!###############################################################################
  pure function evaluate_point( distribs_container, &
    ...
    !! Viability of the test point for 2-, 3-, and 4-body interactions.
       ...
       atom_loop: do ia = 1, basis%spec(is)%num, 1
          ...
          associate( position_store => [ basis%spec(is)%atom(ia,1:3) ] )
             ...
             if( bondlength .ge. tolerances(3) .and. &
                ...
                rtmp1 = sin( sin_scale2 * ( bondlength - tolerances(3) ) ) ** 2._real32
                if( isnan(rtmp1) .or. rtmp1 .lt. 0.01_real32 )then
                   neighbour_basis%image_spec(is)%num = &
                        neighbour_basis%image_spec(is)%num - 1
                else
                   ...
                   rnum_4body_power = rnum_4body_power + rtmp1
                end if
             end if
    ...
    !---------------------------------------------------------------------------
    if(rnum_2body.lt.1.E-2_real32)then
    ...
    if(rnum_4body_power .lt. 5.E-2_real32)then
       has_4body = .false.
       rnum_4body_power = 1._real32
    else
       has_4body = .true.
       rnum_4body_power = 1._real32 / rnum_4body_power
    end if
    do is = 1, is_end, 1
          ...
          if( has_4body )then
             do js = is, neighbour_basis%nspec
                ...
                do ja = ja_start, neighbour_basis%spec(js)%num, 1
                   ...
                   ! other atoms
                   !------------------------------------------------------------
                   rnum_4body = rnum_4body + &
                        position_2(4) * neighbour_basis%spec(js)%atom(ja,4)
                   viability_4body = viability_4body * max( &
                        ...
                        evaluate_4body_contributions( distribs_container, &
                             ...
                             neighbour_basis, element_idx, rnum_4body_power &
                        ) )
                end do
             end do
          end if
...
!###############################################################################
  pure function evaluate_4body_contributions( distribs_container, &
       position_1, position_2, position_3, basis, element_idx, power &
  ) result(output)
    ...
    integer :: bin
    !! Bin for the distribution function.
    real(real32) :: rtmp1, weight_2_3
    !! Temporary variables.


    output = 1._real32
    weight_2_3 = position_2(4) * position_3(4)
