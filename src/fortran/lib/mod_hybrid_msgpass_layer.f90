module raffle__hybrid_msgpass_layer
  !! Hybrid MLIP-inspired message passing layer for RAFFLE.
  !!
  !! Implements a physics-biased message passing layer that combines:
  !!   1. Gaussian RBF expansion with cosine cutoff
  !!   2. Continuous-filter convolution via kernel MLP
  !!   3. Bypass connection: W @ h (like MLIP)
  !!   4. Residual: h' = activation(agg + W @ h + b) + h
  !!
  !! This layer is designed for MLIP-grade accuracy with explicit
  !! physics inductive biases (distance decay, local env descriptors)
  !! plus a residual connection for deep stability.
  !!
  !! Message update (per time step t):
  !!   rbf = expand_rbf(distances)
  !!   for each node i:
  !!     msg_i = sum_j kernel(rbf_ij) @ h_j
  !!     bypass = W_out @ h_i
  !!     delta = sigma(msg_i + bypass + b_out)
  !!     h_i' = h_i + delta
  ...
  public :: hybrid_msgpass_layer_type

  real(real32), parameter :: PI_VAL = 4.0_real32 * atan(1.0_real32)

  type, extends(msgpass_layer_type) :: hybrid_msgpass_layer_type
     !! Hybrid MLIP-inspired with bypass and residual.
     integer :: n_rbf = 20
   ...
   contains
     procedure, pass(this) :: get_num_params => get_num_params_hybrid
     procedure, pass(this) :: set_hyperparams => set_hyperparams_hybrid
     procedure, pass(this) :: init => init_hybrid
     procedure, pass(this) :: print_to_unit => print_to_unit_hybrid
     procedure, pass(this) :: read => read_hybrid
     procedure, pass(this) :: update_message => update_message_hybrid
     procedure, pass(this) :: update_readout => update_readout_hybrid
  end type hybrid_msgpass_layer_type

  interface hybrid_msgpass_layer_type
     module procedure layer_setup_hybrid
  end interface hybrid_msgpass_layer_type
...
!###############################################################################
  pure function get_num_params_hybrid(this) result(num_params)
    !! Get the total number of learnable parameters.
    !!
    !! Per time step t:
    !!   - Kernel MLP: H*n_rbf + H + F*H + F  (F = F_v * F_v)
    !!   - W_out: F_v * F_v + F_v  (output weight + bias)
    !! Readout per time step:
    !!   - Readout W:  num_outputs * F_v
    implicit none
    class(hybrid_msgpass_layer_type), intent(in) :: this
    integer :: num_params
    ...
    ! Per step: kernel_MLP + out(W+b)
    ! Gate (W+b) stored separately (non-learnable)
    num_params = nsteps * ( &
  ...
  end function get_num_params_hybrid
!###############################################################################


!###############################################################################
  function layer_setup_hybrid( &
    ...
    type(hybrid_msgpass_layer_type) :: layer
  ...
  end function layer_setup_hybrid
!###############################################################################


!###############################################################################
  subroutine set_hyperparams_hybrid( &
    ...
    use athena__initialiser, only: initialiser_setup, get_default_initialiser
    implicit none
    class(hybrid_msgpass_layer_type), intent(inout) :: this
    integer, dimension(:), intent(in) :: num_vertex_features
    integer, dimension(:), intent(in) :: num_edge_features
    integer, intent(in) :: num_time_steps
    integer, intent(in) :: num_outputs
  ...
  end subroutine set_hyperparams_hybrid
!###############################################################################


!###############################################################################
  subroutine init_hybrid(this, input_shape, verbose)
    !! Initialise the Hybrid layer: allocate and initialise parameters.
    !!
...
!###############################################################################
  subroutine expand_rbf_features_hybrid(distances, n_rbf, cutoff, rbf_vals)
  ...
  end subroutine expand_rbf_features_hybrid
!###############################################################################


!###############################################################################
  subroutine update_message_hybrid(this, input)
    !! Update messages for the Hybrid MLIP-inspired layer.
    !!
    !! For each sample s and time step t:
    !!   1. Expand edge distances to RBF features
    !!   2. Evaluate kernel MLP on RBF-expanded edge features
    !!   3. Aggregate neighbour messages using per-edge kernels
    !!   4. Bypass: W_out @ h (transform input features like MLIP)
    !!   5. Combine: z = agg + bypass + bias → activation
    !!   6. Residual: h' = activated + h
    !!   7. Store intermediate embedding for readout
    implicit none
    class(hybrid_msgpass_layer_type), intent(inout), target :: this
    ...
    type(array_type), pointer :: ptr_bypass, ptr_combined, ptr_biased
    type(array_type), pointer :: ptr_activated
    ...
    do s = 1, num_samples
       ...
       call expand_rbf_features_hybrid( &
       ...
       do t = 1, nsteps
          ...
          ! Step 3: Bypass - transform input features (like MLIP)
          ptr_bypass => matmul(this%params(p + 2), ptr_h)

          ! Step 4: Combine aggregation + bypass
          ptr_combined => ptr_agg + ptr_bypass

          ! Step 5: Add bias
          ptr_biased => add_bias( &
               ptr_combined, this%params(p + 3), dim=1, dim_act_on_shape=.true.)
  ...
  end subroutine update_message_hybrid
!###############################################################################


!###############################################################################
  subroutine update_readout_hybrid(this)
  ...
  end subroutine update_readout_hybrid
!###############################################################################


!###############################################################################
  subroutine print_to_unit_hybrid(this, unit)
  ...
  end subroutine print_to_unit_hybrid
!###############################################################################


!###############################################################################
  subroutine read_hybrid(this, unit, verbose)
    implicit none
    class(hybrid_msgpass_layer_type), intent(inout) :: this
    integer, intent(in) :: unit
    integer, intent(in), optional :: verbose
    ...
    call stop_program("Hybrid layer read not yet implemented")

  end subroutine read_hybrid
!###############################################################################


end module raffle__hybrid_msgpass_layer
