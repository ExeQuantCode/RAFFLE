module help
  ...
  use geom, only: get_bondlength, get_bondangle
...
!!! but could be swapped around.  
  ...
  ! real(real12) :: value_return, summation
  ...
  !             !write(*,*) "4_body set to zero on", i, j, k 
  !          end if
...
real(real12) :: theta, x
real(real12), dimension(3) :: A,B,C,D, bond1, bond2, AB, AC, AD
!write(*,*) ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"
!write(*,*) A, B, C, D
AB(:)=B(:)-A(:)
AC(:)=C(:)-B(:)
...
!write(*,*) eltot, tmpstochio, tmpelnames 


!write(*,*) "The new atom total is", leng
...
do while (i.ge.1)
 ...
 if((tmpint-stochio(i)).gt.0) then
    !write(*,*) (tmpint-stochio(i)), tmpint, stochio(i)
    !write(*,*) array(structures,atomnumber)%name, array(structures,tmpint-stochio(i))%name
    if(array(structures,atomnumber)%name.ne.(array(structures,tmpint-stochio(i))%name)) then 
       tmpint=tmpint-stochio(i)
    end if
    i=i+1
    !write(*,*) tmpint+stochio(i),tmpint, i
       ...
       do k=1, eltot
          do L=1, stochio(k)
             do n=1, 27
                ...
                !     &.AND.(get_bondangle(repeatedarray(structures,m)%position,&
                !     &array(structures,atomnumber)%position,&
                ...
                if(get_bondangle(repeatedarray(structures,m)%position,&
                   ...
                   write(101,*) get_bondangle(repeatedarray(structures,m)%position,&
                        &array(structures,atomnumber)%position,&
                        &repeatedarray(structures,x)%position)
                   ...
                   write(102,*) 3.141592654 - get_bondangle(repeatedarray(structures,m)%position,&
                        &array(structures,atomnumber)%position,&
                        &repeatedarray(structures,x)%position)
                   ...
                   write(name2, '(A,A,I0.3,A1,A,A,A,A,A,A,I0.3)') &
                        ...
                        &trim(adjustl(structure_elements(bondint_3)))
                   open(102,file=name2, position="append")
                   write(102,*) get_bondangle(repeatedarray(structures,m)%position,&
                        &array(structures,atomnumber)%position,&
                        &repeatedarray(structures,x)%position)
                end if


                ! write(*,*) bondlengthcutoff(bondint_1,bondint_2), structure_elements(bondint_1), structure_elements(bondint_2)
                ! write(*,*) bondlengthcutoff(bondint_1,bondint_3), structure_elements(bondint_1), structure_elements(bondint_3)
                ...
                !   write(*,*) "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
                !   write(*,*) repeatedarray(structures,m)%position
                !   write(*,*) array(structures,atomnumber)%position
                !   write(*,*) repeatedarray(structures,x)%position
                !   write(*,*) get_bondangle(repeatedarray(structures,m)%position,&
                !     &array(structures,atomnumber)%position,&
                !     &repeatedarray(structures,x)%position)
                !   write(*,*) array(structures,atomnumber)%position-repeatedarray(structures,m)%position
                !   write(*,*) array(structures,atomnumber)%position-repeatedarray(structures,x)%position
                !   stop 
                !end if
             end do
...
do x=-1,1
 do y=-1,1
    do z=-1,1
       do j=1, 3
          !write(*,*) array(structures,m)%position(j),position(j)
          ...
          !write(*,*)   array(structures,m)%position(j)
       end do
...
do x=-1,1
 do y=-1,1
    do z=-1,1
       ...
       !write(*,*) array(structures,m)%position(:)
       !stop
       m=m+1
    end do
 end do
