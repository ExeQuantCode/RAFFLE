program test_generator
  ...
  basis_host_expected%spec(1)%atom(9, :3) = [0.0, 0.0, 0.5]
  basis_host_expected%spec(1)%atom(10, :3) = [0.5, 0.5, 0.5]
  basis_host_expected%spec(1)%atom(11, :3) = [0.75, 0.25, 0.875]
  basis_host_expected%spec(1)%atom(12, :3) = [0.25, 0.75, 0.875]
  basis_host_expected%spec(1)%atom(13, :3) = [0.0, 0.5, 0.75]
  basis_host_expected%spec(1)%atom(14, :3) = [0.5, 0.0, 0.75]
  basis_host_expected%spec(1)%atom(15, :3) = [0.75, 0.75, 0.625]
  basis_host_expected%spec(1)%atom(16, :3) = [0.25, 0.25, 0.625]
      ...
      do ia = 1, bas1%spec(is)%num
         if( &
              any( &
                   ...
                   ) .ge. tolerance + 1.E-6_real12 &
              ) &
         ) then
            write(0,*) 'Generator failed to produce expected atom'
